...
// SITE_ROOT is the published site; OUT_DIR is where this tool's own artefacts go.
// They were one constant back when this script lived in tests/, where '..' meant
// the repo root. Moving it to _dev/qa/ silently repointed that '..' at _dev, so
// page discovery walked _dev instead of the site: every run crawled the content
// backups under _dev/content-originals, all 89 of them 404ed on the dev server,
// and the report came back "89 critical, 0 clean pages" without ever having
// loaded a real page. Keep the two apart so a future move cannot repeat it.
const SITE_ROOT = path.resolve(__dirname, '../..');
const OUT_DIR = path.resolve(__dirname, '..');
const SCREENSHOTS_DIR = path.join(OUT_DIR, 'qa-screenshots');
const REPORT_HTML = path.join(OUT_DIR, 'qa-report.html');
const REPORT_MD = path.join(OUT_DIR, 'qa-report.md');
...
function discoverPages() {
  ...
  function walk(dir) {
    const SKIP_DIRS = new Set(['node_modules', 'tests', 'docs', 'design-brainstorm', 'tmp', 'partials', 'pages', 'css', 'js', 'fonts', 'images', '_dev', 'blog-backup', 'backup']);
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        const rel = path.relative(SITE_ROOT, path.dirname(full));
...
async function checkPage(page, urlPath, issuesBucket) {
  ...
  // Resolve links against where the browser ENDED UP, not what we asked for.
  //
  // `/` is a language splash whose inline script does
  // window.location.replace('/fr/'), so the crawler requests '/' and lands on
  // '/fr/'. Resolving that page's relative hrefs against the REQUESTED url
  // turned every one of them into a root-level path: 'cours-de-tango/' became
  // /cours-de-tango/ instead of /fr/cours-de-tango/. All 11 then 404ed, and the
  // report opened with a table of broken links that were not broken — checked
  // against production 2026-09-08, every real /fr/... URL returns 200. (#1172)
  //
  // Same shape as the SITE_ROOT bug documented at the top of this file: a base
  ...
  // `el.href` (not getAttribute) is resolved by the DOM against the document's
  // own base URI — the browser's answer, and it honours a <base> tag too.
  ...
  for (const href of hrefs) {
    try {
      const resolved = new URL(href, landedUrl);
      if (resolved.origin !== baseOrigin) continue;
      if (checked.has(resolved.href)) continue;
      checked.add(resolved.href);

      // Skip anchors-only
      if (resolved.pathname === new URL(landedUrl).pathname && resolved.hash) continue;
...
function generateReports(results) {
  ...
  const screenshotGrid = results.map(r => {
    ...
    const mExists = fs.existsSync(path.join(OUT_DIR, m));
    const tExists = fs.existsSync(path.join(OUT_DIR, t));
    return `
